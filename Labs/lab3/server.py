...
class Server(object):
    ...
    def _bind(self):
        ...
        :return: VOID
        """
        #Binding the server socket the Host and Port
    ...
    def _accept_clients(self):
        ...
        while True:
            ...
            #Receiving the data, allocating the max buffer size
            data = clienthandler.recv(self.MAX_ALLOC_BUFFER)
            
            #Deserializing the data
            deserialized_data = pickle.loads(data)
            
            #Calling Process_Request
            self._process_request(clienthandler,deserialized_data)
    ...
    def _process_request(self, clienthandler, request):
        ...
        if clienthandler:
            print(f'Connected: Student: {request["student_name"]}, Github Username: {request["github_username"]}, sid: {request["sid"]}')
            
        else:
            print("Failed to processm, {}".format(clienthandler))
